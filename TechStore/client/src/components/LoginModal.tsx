...
import { X, Lock, Mail, AlertCircle, LogIn, Shield, User, ArrowRight, UserPlus, Phone, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="modal-overlay" onClick={closeLoginModal} role="dialog" aria-modal="true">
      <div
        ...
        style={{ maxWidth: '480px', padding: '30px 26px' }}
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-white font-heading">
                {modo === 'login' ? 'Iniciar Sesión' : 'Crear Nueva Cuenta'}
              </h3>
              <p className="text-xs text-slate-400">Plataforma Oficial TechStore Bolivia</p>
            </div>
          </div>
          <button
        ...
        {registroExitoso && (
          <div className="flex items-center gap-2.5 text-xs text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 p-3.5 rounded-xl mb-5">
        ...
        {modo === 'login' ? (
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-slate-400 mt-1">
                Contraseña por defecto para cuentas iniciales: <span className="text-cyan-400 font-mono font-semibold">123</span>
              </p>
            </div>
            ...
            >
              <span>{cargando ? 'Verificando...' : 'Ingresar a mi cuenta'}</span>
              <ArrowRight size={16} />
            </button>
          </form>
        ) : (
          /* MODE 2: REGISTER FORM ("Crear Cuenta") */
          <form onSubmit={handleRegisterSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full mt-4 flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold text-sm shadow-glow transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
            >
              <span>{cargando ? 'Registrando...' : 'Completar Registro'}</span>
              <ArrowRight size={16} />
            </button>
          </form>
        )}
